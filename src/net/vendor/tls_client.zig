...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    // Advertise HTTP/2 (ALPN "h2"). Fixed comptime constant so the
    // comptime client_hello construction below stays compile-time.
    const alpn_extension = blk: {
        const ext = int(u16, @intFromEnum(tls.ExtensionType.application_layer_protocol_negotiation)) ++
            int(u16, 5) ++
            int(u16, 3) ++
            .{ 2, 'h', '2' };
        break :blk ext;
    };
    const alpn_extension_len = alpn_extension.len;
...
/// Access to the decrypted (plaintext) read stream.
pub fn plaintextReader(c: *Client) *Reader {
    return &c.reader;
}

/// Access to the plaintext write stream.
pub fn plaintextWriter(c: *Client) *Writer {
    return &c.writer;
}
